...
def plot_psd(t_data_crop, fsample, save_folder, csv_number, exp_name, save_figs):
    try:
        ...
        psdtrace_time = np.arange(N_psd)/fsample # in s
        ...
        psd_y.mlefit()
        psd_c0int.mlefit()
        psd_results_x = psd_x.results
        psd_results_y = psd_y.results
        psd_results_c0int = psd_c0int.results
        x_df = pd.DataFrame([psd_results_x])
        x_df['csv_number'] = csv_number
        x_df['component'] = 'x'
        y_df = pd.DataFrame([psd_results_y])
        y_df['csv_number'] = csv_number
        y_df['component'] = 'y'
        c0int_df = pd.DataFrame([psd_results_c0int])
        c0int_df['csv_number'] = csv_number
        c0int_df['component'] = 'c0int'
        # concatenates them all together
        df_combined = pd.concat([x_df, y_df, c0int_df], ignore_index=True)
